...
Matrix Exponentiation in c++ is basicalong longy  a concept borrowed from/in Mathematics.
...
Hence ,,Sometimes we might  face some problems in general as welong long as in cpp, where we can easily derive a recursive relation, (mostly/generalong longy  suitable for dynamic programming also calong longed (dp) approach),but solving with the same linearly,,generalong longy might give us TLE, for l,arge values of N(when we have to find the Nth term of the relation). That is where Matrix Exponentiation comes into play and here it plays a key value ,in solving problem.
...
F(i)=a11*(F(i-1))+a12*(F(i-2))+a13*(F(i-3)).. +a1k*(F(i-k))    ->can be solved logicalong longy
