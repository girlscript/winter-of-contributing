...
***Node.js*** is a popular JS runtime environment for creating web applications on server side, that is it allows us to run JS on the backend outside the browser. It comes with a lot of built-in modules and it enables us to run and test JS code very fast and efficiently mainly because of its asynchronous event run model running on Google V8 engine.
<br>
...
- Open any text editor and create a JS file with any piece of JS sample code. Like, here, I used VS code to create the js file with the code shown. 
...
Some popular NodeJs frameworks for web-applications are ***Express.JS, Feather.js, Meteor.js, Total.js, Socket.io, Nest.js, Hapi.js, Sails.js etc***.
