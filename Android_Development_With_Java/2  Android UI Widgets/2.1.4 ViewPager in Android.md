...
To display a different animation from the default slideshow display, use the interface of `ViewPager.PageTransformer` and render it to the viewer pager. The interface identifies one method, `transformPage ()`. At each turn of the screen, this option is called once per page visible (usually only one page visible) and nearby pages outside the screen. For example, if the third page appears and the user drags to the fourth page, `transformPage ()` is called two, three, and four pages per step of action.
